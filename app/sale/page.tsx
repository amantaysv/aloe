...
export default async function SalePage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-8">
      ...
      {products.length === 0 ? (
        <p className="text-gray-400 text-sm">Акций нет</p>
      ) : (
        <>
              ...
              <div key={p.id} className="border rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative aspect-square bg-gray-50">
                  ...
                  <div className="absolute top-1.5 left-1.5">
                    <span className="bg-orange-500 text-white text-[10px] font-semibold px-1.5 py-0.5 rounded">Акция</span>
                  </div>
          ...
          <Pagination page={currentPage} totalPages={totalPages} basePath="/sale" />
        </>
      )}
    </main>
  );
